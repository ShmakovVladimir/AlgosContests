...
class Network:
    ...
    def readGraph(self):
        ...
        self.networkEdges = []
        self.vertexEdgesIndexes = [[] for _ in range(self.vertexQ)]
        for i in range(self.edgeQ):
            vertex1,vertex2,capasity = map(int,input().split())
            self.networkEdges.append(self.Edge(vertex1,vertex2,capasity))
            self.networkEdges.append(self.Edge(vertex2,vertex1,0))
            self.networkEdges[-1].invertEdge = self.networkEdges[-2]
            self.networkEdges[-2].invertEdge = self.networkEdges[-1]
            self.vertexEdgesIndexes[vertex1].append(2*i)
            self.vertexEdgesIndexes[vertex2].append(2*i+1)
    def findPathAndCapasity(self):
        def DFS(start: int):
            visited[start] = True
            for vertexCode in self.vertexEdgesIndexes[start]:
                edge = self.networkEdges[vertexCode]
